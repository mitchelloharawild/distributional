test_that("Multivariate normal distribution", {
  ...
  # pdf
  expect_equal(density(dist, cbind(1, 2)), 0.0562697697598191) # dput(mvtnorm::dmvnorm(c(1, 2), mean = mu, sigma = sigma))
  expect_equal(density(dist, cbind(-3, 4)), 0.000139478814272667) # dput(mvtnorm::dmvnorm(c(-3, 4), mean = mu, sigma = sigma))

  # cdf
  expect_equal(cdf(dist, cbind(1, 2)), 0.347956638007652) # dput(mvtnorm::pmvnorm(upper = c(1,2), mean = mu, sigma = sigma))
  expect_equal(cdf(dist, cbind(-3, 4)), 0.124078569373413) # dput(mvtnorm::pmvnorm(c(-3, 4), mean = mu, sigma = sigma))
